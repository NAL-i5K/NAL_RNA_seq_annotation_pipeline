...
urlretrieve(
    'https://github.com/broadinstitute/picard/releases/download/3.0.0/picard.jar',
    join(lib_dir, 'picard.jar'))

print('Downloading GATK v4 ...')
urlretrieve('https://github.com/broadinstitute/gatk/releases/download/4.4.0.0/gatk-4.4.0.0.zip',
    join(lib_dir, 'gatk-4.4.0.0.zip'))
...
with ZipFile(join(lib_dir, 'fastqc_v0.12.1.zip'), 'r') as zip_ref:
...
with ZipFile(join(lib_dir, 'Trimmomatic-0.39.zip'), 'r') as zip_ref:
    zip_ref.extractall(lib_dir)
...
with ZipFile(join(lib_dir, 'gatk-4.4.0.0.zip'), 'r') as zip_ref:
    zip_ref.extractall(lib_dir)

print('Cleaning the files ...')
files = [
    'BBMap_38.00.tar.gz', 'fastqc_v0.12.1.zip',
    'hisat2-2.1.0-Linux_x86_64.zip', 'Trimmomatic-0.39.zip',
    'gatk-4.4.0.0.zip'
]
for f in files:
