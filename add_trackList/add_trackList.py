...
parser.add_argument("-op","--output_path", help="path of gggsss directory - take this from 'directory' column in  https://github.com/NAL-i5K/jbrowse_track_configs/blob/main/organisms_orig/directory_list.orig, and remove jbrowse/data, e.g. /app/data/other_species/aettum/
...
new_dir_path = path.join(args.output_path, 'scaffold', 'analyses', folder_name)
...
src_dir = path.join(args.output_path, 'scaffold', 'analyses')
dst_dir = path.join(args.output_path, 'jbrowse/data/analyses')
if not path.exists(dst_dir):
    print('Create a symlink')
    os.symlink(src_dir, dst_dir)

#backup trackList in apollo directory
shutil.copyfile(path.join(args.output_path, 'jbrowse/data', 'trackList.json'), path.join(args.output_path, 'jbrowse/data', 'trackList.json.bak'))
...
metadata = '{"unsafePopup": true, "metadata": {"Analysis provider": "i5k Workspace@NAL",\n"Analysis method": "https://github.com/NAL-i5K/NAL_RNA_seq_annotation_pipeline/",\n"Data source":"' + ','.join(data_source_runs) + '",\n"Publication status":"Analysis: NA; Source data: see individual SRA accessions",\n"Track legend":"Junction reads generated by Hisat2 aligner and regtools. The number of alignments supporting the junction read is listed in blue."}, "category":"RNA-Seq/Splice junctions" }'
description = '{ "description": "score" }'
proc_junc = subprocess.Popen(['singularity', 'exec', args.jbrowse_path, 'flatfile-to-json.pl', '--bed', path.join(new_dir_path, args.input_bed), '--trackLabel', label + '_junctions', '--config', metadata, '--out', path.join(args.output_path, 'jbrowse/data'), '--clientConfig', description])
