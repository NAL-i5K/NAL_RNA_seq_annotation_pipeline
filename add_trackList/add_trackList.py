...
parser.add_argument("-s","--Source", help="Source file name",type=str)
...
metadata = '{"style":{"showLabels": false}, "metadata": {"Analysis provider": "i5k Workspace@NAL",\n"Analysis method": "https://github.com/NAL-i5K/NAL_RNA_seq_annotation_pipeline/",\n"Data source":"' + ','.join(Submission) + '",\n"Publication status":"Analysis: NA; Source data: see individual SRA accessions",\n"Track legend":"Intronic junction reads generated by Hisat2 aligner and regtools"}, "category":"RNA-Seq/Intronic splice junctions" }'
proc_junc = subprocess.Popen(['flatfile-to-json.pl', '--bed', path.join(new_dir_path, args.input_bed), '--trackLabel', label[0:-10] + datetime.datetime.now().strftime("%Y-%m-%d") + '_junctions', '--config', metadata, '--className', 'feature3', '--out', path.join('/app/data/other_species', gggsss, assembly_name, 'jbrowse/data')])
