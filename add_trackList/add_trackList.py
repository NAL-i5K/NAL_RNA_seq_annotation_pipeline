...
proc_junc = subprocess.Popen(['singularity', 'exec', args.jbrowse_path, 'flatfile-to-json.pl', '--bed', path.join(new_dir_path, args.input_bed), '--trackLabel', label + '_junctions', '--config', metadata, '--out', path.join(args.output_path, gggsss, assembly_name, 'jbrowse/data'), '--clientConfig', description])
