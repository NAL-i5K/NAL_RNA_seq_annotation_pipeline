...
parser.add_argument("-op","--output_path", help="path of gggsss directory where jbrowse files live (e.g. /app/data/other_species or /project/genomics/username/species_files)",type=str)              
...
new_dir_path = path.join(args.output_path, gggsss, assembly_name, 'scaffold', 'analyses', folder_name)
...
metadata = '{"unsafePopup": true, "style":{"showLabels": false, "description":"score"}, "metadata": {"Analysis provider": "i5k Workspace@NAL",\n"Analysis method": "https://github.com/NAL-i5K/NAL_RNA_seq_annotation_pipeline/",\n"Data source":"' + ','.join(data_source_runs) + '",\n"Publication status":"Analysis: NA; Source data: see individual SRA accessions",\n"Track legend":"Junction reads generated by Hisat2 aligner and regtools. The number of alignments supporting the junction read is listed in blue."}, "category":"RNA-Seq/Splice junctions" }'
proc_junc = subprocess.Popen(['singularity', 'exec', args.jbrowse_path, 'flatfile-to-json.pl', '--bed', path.join(new_dir_path, args.input_bed), '--trackLabel', label + '_junctions', '--config', metadata, '--out', path.join(args.output_path, gggsss, assembly_name, 'jbrowse/data')])
