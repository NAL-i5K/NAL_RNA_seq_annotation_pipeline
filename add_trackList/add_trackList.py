...
proc_bam = subprocess.Popen(['add-bam-track.pl', '-i', args.input_track, '-o', args.input_track, '--bam_url', path.join('analyses', folder_name, args.input_bam), '--label', label, '--category', 'RNA-Seq/Mapped Reads', '--config', metadata])
...
proc_bw = subprocess.Popen(['add-bw-track.pl', '-i', args.input_track, '-o', args.input_track, '--bw_url', path.join('analyses', folder_name, args.input_bigwig), '--label', label + '_coverage', '--plot', '--category', 'RNA-Seq/Coverage Plots', '--pos_color', "#00BFFF", '--config', metadata])
proc_bw.communicate()
...
proc_junc = subprocess.Popen(['flatfile-to-json.pl', '--bed', path.join(new_dir_path, args.input_bed), '--trackLabel', label + '_junctions', '--config', metadata, '--out', path.join('/app/data/other_species', gggsss, assembly_name, 'jbrowse/data')])
